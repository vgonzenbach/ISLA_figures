...
# find order of factors for plotting
roi_order = roi_df %>% 
  filter(method  == 'Uncorrected', radius == 'R = 2') %>% 
  group_by(REGION) %>% 
  arrange(desc(`PROP. SIG. P-VALUES`), .by_group = TRUE) %>% 
  select(ROI_NAME) %>%
    ungroup() %>% 
    pull() %>% 
    as.character()
...
roi_df %>% 
  mutate(ROI_NAME = factor(ROI_NAME, levels=roi_order)) %>% # re-order factors for plotting
  ggplot(aes(x=ROI_NAME, y=`PROP. SIG. P-VALUES`, fill=method)) + 
  geom_bar(position = position_dodge(-.9), stat='identity') + 
  facet_grid(REGION~radius, scales = 'free', space = "free") + 
  coord_flip() + 
  scale_fill_calc(name = "Method") +
  xlab('ROI') +
  ylab('Proportion Sig. P-values') + 
  theme_bw() + 
  theme(strip.background = element_rect(color="gray", fill="white", size=1.5, linetype="solid"))
```



```{r}
roi_df = read.csv(here::here('results/data/roi_gmd.csv')) %>% 
  rename(ROI_INDEX = X) %>% 
  right_join(roi_df, by = 'ROI_INDEX')
```
